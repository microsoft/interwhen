...
VLLM_PORTS = [8000, 8001, 8002, 8003]  # 4 instances
...
def build_full_prompt(task, example, nums=None):
    ...
    if task == "verina":
        return build_verina_prompt(example)  # example is BenchmarkData for verina
    if task == "maze":
...
def parse_critic_verdict(critic_output: str) -> Tuple[bool, str]:
    """
    Robustly parse CORRECT/INCORRECT verdict from critic output.
    
    Handles:
    - <think>...</think> blocks (only looks at content after thinking)
    - Various formats: VERDICT: CORRECT, **CORRECT**, etc.
    - Multiple verdicts (takes the last one)
    - Case insensitivity
    """
    # Step 1: Remove <think>...</think> blocks to focus on the final answer
    cleaned = re.sub(r'<think>.*?</think>', '', critic_output, flags=re.DOTALL | re.IGNORECASE)
    
    # If </think> exists but removal didn't work (partial), take everything after </think>
    if not cleaned.strip():
        think_end = critic_output.lower().rfind("</think>")
        if think_end != -1:
            cleaned = critic_output[think_end + len("</think>"):]
        else:
            cleaned = critic_output
    
    # Step 2: Try to find explicit VERDICT pattern (most reliable)
    # Matches: "VERDICT: CORRECT", "VERDICT: INCORRECT", "**VERDICT:** CORRECT", etc.
    verdict_patterns = [
        r'VERDICT\s*[:\-]?\s*\**\s*(INCORRECT|CORRECT)\b',  # VERDICT: CORRECT or VERDICT: INCORRECT
        r'\*\*VERDICT\*\*\s*[:\-]?\s*\**\s*(INCORRECT|CORRECT)\b',  # **VERDICT**: CORRECT
        r'(?:^|\n)\s*\**\s*(INCORRECT|CORRECT)\s*\**\s*(?:$|\n)',  # Standalone CORRECT/INCORRECT on a line
    ]
    ...
    for pattern in verdict_patterns:
        ...
        if matches:
            # Take the last match (in case model changes its mind)
            verdict = matches[-1].group(1).upper()
            break
    
    # Step 3: If no explicit verdict found, check for keywords more carefully
    if verdict is None:
        upper_cleaned = cleaned.upper()
        
        # Find last occurrence of each keyword
        # Note: \b word boundaries ensure CORRECT inside INCORRECT won't match
        last_incorrect = upper_cleaned.rfind('INCORRECT')
        
        # Find last standalone CORRECT (not part of INCORRECT)
        last_correct = -1
        for match in re.finditer(r'\bCORRECT\b', upper_cleaned):
            last_correct = match.start()
        
        if last_incorrect > last_correct:
            verdict = "INCORRECT"
        elif last_correct > last_incorrect:
            verdict = "CORRECT"
        # If neither found or equal position, default will apply
    
    is_correct = (verdict == "CORRECT")
    
    # Step 4: Extract reasoning
    reasoning = ""
    if "REASONING:" in cleaned:
        reasoning = cleaned.split("REASONING:", 1)[1].strip()
    elif "EXPLANATION:" in cleaned.upper():
        # Handle EXPLANATION: variant
        split_result = re.split(r'EXPLANATION\s*:', cleaned, flags=re.IGNORECASE, maxsplit=1)
        if len(split_result) > 1:
            reasoning = split_result[1].strip()
    elif verdict:
        # Take everything after the verdict line as reasoning
        verdict_match = re.search(r'VERDICT\s*[:\-]?\s*\w+', cleaned, re.IGNORECASE)
        if verdict_match:
            reasoning = cleaned[verdict_match.end():].strip()
    
    # Clean up reasoning - remove leading colons, dashes, etc.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, required=True, choices=["game24", "maze", "spatialmap", "verina"],
                        help="Task to run")
