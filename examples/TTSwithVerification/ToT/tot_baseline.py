...
from utils.tree_of_thought import (
    SearchMethod,
    ToTSearchConfig,
    TreeOfThoughtSearch,
    build_tot_problem,
...
from utils.zebralogic_helper import extract_last_json, zebra_correctness
from utils.verina_tot_utils import (
    load_verina_dataset,
    extract_code_from_response,
    evaluate_generated_code,
)
from utils.verina_spec_tot_utils import (
...
_TASK_FOLDER_MAP = {
    "game24": "Gameof24results",
    "maze": "MazeResults",
    "spatialmap": "SpatialMapResults",
    "zebralogic": "zebralogic",
    "verina": "verina",
    "verina_spec": "verina_spec",
}
...
def get_default_output_dir(task: str, model: str) -> str:
    """Build the default output directory matching the project convention."""
    task_folder = _TASK_FOLDER_MAP.get(task, task)
    model_short = get_model_short_name(model)
    return os.path.join(_OUTPUT_ROOT, "Outputs_TTS", "ToT", task_folder, model_short)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--llm_url", default="http://localhost:{port}/v1/completions")
    parser.add_argument(
        "--ports",
        default="8000",
    ...
    parser.add_argument(
        "--output_dir",
        default=None,
        help="Directory to store per-example JSON logs and summary (auto-derived if omitted)",
    )
    ...
    if args.output_dir is None:
        args.output_dir = get_default_output_dir(args.task, args.model)
    return args
...
async def run_single_example(
    ...
    if task == "verina" and best_traj.strip():
        try:
            ...
            synthesized_code = await stream_completion(
                synthesis_prompt,
        ...
        try:
            ...
            synthesized_spec = await stream_completion(
                synthesis_prompt,
                llm_server=llm_server,
                monitors=[],
                add_delay=False,
