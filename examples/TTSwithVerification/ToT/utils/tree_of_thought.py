...
def _build_verina_problem_text(example) -> str:
    """Build the shared verina problem description used by both propose and synthesis prompts."""
    data = example  # BenchmarkData instance
    signature = data.signature
    func_name = signature.get("name", "solution")
    return_type = signature.get("return_type", "Bool")
    params = signature.get("parameters", [])
    param_list = " ".join(
        f"({p['param_name']} : {p['param_type']})" for p in params
    )
...
def build_verina_tot_prompt(example) -> str:
    """Build a plain-text verina problem prompt for ToT (no chat template)."""
    return (
        "You are an expert Lean 4 programmer. Think step-by-step about how to "
        "implement the function body described below.\n\n"
        + _build_verina_problem_text(example)
    )


def build_verina_synthesis_prompt(problem: str, trajectory: str) -> str:
    """Build a prompt that converts a reasoning trajectory into final Lean 4 code."""
    return (
        "You are an expert Lean 4 programmer. Based on the reasoning below, "
        "produce the final function body expression. Output ONLY the code "
        "wrapped in [CODE]...[/CODE] tags — no explanation.\n\n"
        f"PROBLEM:\n{problem}\n\n"
        f"REASONING TRAJECTORY:\n{trajectory}\n\n"
        "Write the final Lean 4 function body expression inside [CODE]...[/CODE] tags."
    )


def _build_verina_spec_problem_text(example) -> str:
    """Build the shared verina spec problem description used by both propose and synthesis prompts."""
    data = example  # BenchmarkData instance
    signature = data.signature
    func_name = signature.get("name", "solution")
    return_type = signature.get("return_type", "Bool")
    ...
    # Natural language spec descriptions
    ...
    return (
        ...
        f"def {postcond_name} {param_list} (result: {return_type}) "
        f"(h_precond : {precond_name} {param_names_str}) : Prop :=\n"
        f"  [POSTCOND] -- your postcondition here [/POSTCOND]\n```\n"
        f"{spec_desc_section}{helper_section}{code_section}"
    )


def build_verina_spec_tot_prompt(example) -> str:
    """Build a plain-text verina spec-generation problem prompt for ToT."""
    return (
        "You are an expert Lean 4 programmer specializing in formal specifications. "
        "Think step-by-step about how to write the precondition and postcondition "
        "for the function described below.\n\n"
        + _build_verina_spec_problem_text(example)
    )


def build_verina_spec_synthesis_prompt(problem: str, trajectory: str) -> str:
    """Build a prompt that converts a reasoning trajectory into final Lean 4 spec."""
    return (
        "You are an expert Lean 4 programmer specializing in formal specifications. "
        "Based on the reasoning below, produce the final precondition and postcondition. "
        "Output ONLY the spec wrapped in tags — no explanation.\n\n"
        "Use [PRECOND]...[/PRECOND] for the precondition body.\n"
        "Use [POSTCOND]...[/POSTCOND] for the postcondition body.\n"
        "Optionally use [PRECOND_AUX]...[/PRECOND_AUX] and [POSTCOND_AUX]...[/POSTCOND_AUX] "
        "for helper definitions.\n\n"
        "Use correct lean 4 syntax\n"
        "Do not add imports in your generation specs\n\n"
        f"PROBLEM:\n{problem}\n\n"
        f"REASONING TRAJECTORY:\n{trajectory}\n\n"
        "Write the final Lean 4 specification inside the tags above."
    )
...
def build_tot_problem(task: str, example: Dict[str, Any], nums: Optional[List[int]] = None) -> str:
    ...
    if task_lower == "verina":
        return build_verina_tot_prompt(example)
    if task_lower == "verina_spec":
        return build_verina_spec_tot_prompt(example)
...
class TreeOfThoughtSearch:
    ...
    def _build_propose_prompt(
        ...
        if task == "verina":
            return self._build_verina_propose_prompt(problem, trajectory, num_proposals)
        if task == "verina_spec":
            return self._build_verina_spec_propose_prompt(problem, trajectory, num_proposals)
...
..."""
    ...
    def _build_verina_propose_prompt(
        self,
        problem: str,
        trajectory: str,
        num_proposals: int,
    ) -> str:
        """Build reasoning-step proposal prompts for verina Lean 4 code generation."""
        if not trajectory.strip():
            current = "Starting fresh - no progress yet"
            last_step_hint = ""
        else:
        ...
        return f"""You are reasoning step-by-step toward implementing a Lean 4 function body.
Do NOT write any code yet. Only output reasoning about the approach.
...
{current}
{last_step_hint}
Your task: Propose {num_proposals} DISTINCT next reasoning steps.
Each step MUST add NEW information not already present in the reasoning so far. Valid new information includes:
- A specific Lean 4 function name, operator, or syntax not yet mentioned
- An edge case or branch not yet considered
- A type or signature detail relevant to the implementation
- A postcondition argument not yet made

Do NOT propose steps that merely rephrase, verify, or confirm what is already in the reasoning. "Confirm that X works" or "Verify that Y is correct" are NOT valid proposals unless they add concrete new details.
Do NOT output [CODE] tags. Only reasoning about how to solve the problem.

Output format (one step per line, no preamble):
1. [Reasoning step]
2. [Reasoning step]
..."""
    
    def _build_verina_spec_propose_prompt(
        self,
        problem: str,
        trajectory: str,
        num_proposals: int,
    ) -> str:
        """Build reasoning-step proposal prompts for verina Lean 4 spec generation."""
        if not trajectory.strip():
            current = "Starting fresh - no progress yet"
            last_step_hint = ""
        else:
        ...
        return f"""You are reasoning step-by-step toward writing Lean 4 preconditions and postconditions.
Do NOT write any spec code yet. Only output reasoning about what the precondition and postcondition should capture.
...
Your task: Propose {num_proposals} DISTINCT next reasoning steps.
Each step MUST add NEW information not already present in the reasoning so far. Valid new information includes:
- A specific Lean 4 construct, type, or operator for the spec not yet mentioned
- An input constraint or edge case for the precondition not yet considered
- A postcondition property or invariant not yet identified
- A relationship between input and output that the spec should capture

Do NOT propose steps that merely rephrase, verify, or confirm what is already in the reasoning.
Do NOT output [PRECOND], [POSTCOND], [PRECOND_AUX], or [POSTCOND_AUX] tags. Only reasoning about what the spec should express.
...
..."""
    ...
    def _clean_proposal_text(text: str) -> str:
        """Strip chain-of-thought artifacts (</think>, trailing meta-commentary)."""
        think_idx = text.find("</think>")
        if think_idx != -1:
            text = text[think_idx + len("</think>"):]
    ...
    def _parse_evaluation(self, response: str) -> float:
        ...
        Adds small random noise (±0.05) to break ties between nodes
        that receive the same confidence label.
        """
        ...
        if base_score is None:
            ...
            for i, char in enumerate(response):
                if char.isdigit():
                    digit = int(char)
                    if 1 <= digit <= 9:
                        base_score = digit / 9.0
                        break
        
        if base_score is None:
            base_score = 0.5  # Default neutral score
        
        # Add small uniform noise to break ties
        ...
        for depth in range(self.config.max_depth):
            ...
            for node in queue:
                ...
                for prop in proposals:
                    ...
                    if self._is_terminal(new_trajectory, task):
    ...
    def _is_terminal(self, trajectory: str, task: str = "") -> bool:
        """Check if trajectory represents a complete solution"""
        # Verina: terminal when the trajectory contains a [CODE] block,
        # consistent with how other tasks detect final answers.
        if task == "verina":
            return bool(re.search(r'\[CODE\]', trajectory, re.IGNORECASE))
        # Verina spec: terminal when trajectory contains both [PRECOND] and [POSTCOND].
        if task == "verina_spec":
            has_precond = bool(re.search(r'\[PRECOND\]', trajectory, re.IGNORECASE))
            has_postcond = bool(re.search(r'\[POSTCOND\]', trajectory, re.IGNORECASE))
            return has_precond and has_postcond
        keywords = [
            "final answer",
            "reached goal",
            "solution:",
    ...
    async def _call_llm_streaming(
        ...
        """Call the LLM via interwhen's stream_completion (no monitors)."""
